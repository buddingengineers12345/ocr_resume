...
def find_image() -> Path:
    """Locate the input PNG in image_reference/ folder; falls back to any page*.png."""
...
def load_reference_words(path: Path = None) -> set:
    """
...
def load_reference_order(path: Path) -> list:
...
def coverage(b1: tuple, b2: tuple) -> float:
    """Fraction of box b1 covered by box b2. Each box is (x, y, w, h)."""
    ix1 = max(b1[0], b2[0])
    iy1 = max(b1[1], b2[1])
...
def overlaps_text(x: int, y: int, w: int, h: int,
                  text_boxes: list, threshold: float = 0.25) -> bool:
...
def read_csv_objects(csv_path: Path) -> list:
    """
    Read all rows from the objects CSV.
    Returns a list of dicts with keys: object_type, text, x, y, w, h
    (w/h are used internally; CSV stores them as width/height).
    """
...
def write_csv_objects(objects: list, csv_path: Path):
...
def update_csv_objects(new_objects: list, object_type: str, csv_path: Path):
    """
